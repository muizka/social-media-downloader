...
```bash
pkg update -y && pkg upgrade -y && pkg install python ffmpeg git -y && git clone https://github.com/muizka/social-media-downloader.git; cd ~/social-media-downloader || cd social-media-downloader && pip install -r requirements.txt --break-system-packages && termux-setup-storage
```
